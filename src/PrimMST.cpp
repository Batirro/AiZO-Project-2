...
MSTResult PrimMST::findMST(Graph* graph, int startVertex) {
    ...
    // Dla macierzy incydencji pobieramy krawędzie raz przed pętlą,
    // żeby nie tworzyć kopii DynamicArray przy każdej iteracji.
    DynamicArray incEdges;
    if (incMatrix) {
        incEdges = incMatrix->getEdges();
    }
    ...
    while (!minHeap.isEmpty() && edgesAdded < vertices - 1) {
        ...
        if (adjList) {
            ...
            for (int i = 0; i < incEdges.getSize(); i++) {
                const Edge& edge = incEdges[i];
